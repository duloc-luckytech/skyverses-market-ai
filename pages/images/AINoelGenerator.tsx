...
const AINoelGenerator = () => {
  ...
  useEffect(() => {
    const fetchNoelImages = async () => {
      try {
        const response = await fetch('https://api.skyverses.com/explorer?page=1&limit=10&search=noel');
        const result = await response.json();
        if (result.data) setImages(result.data);
      } catch (error) {
        console.error("Failed to fetch Noel samples:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="bg-[#fdfdfe] dark:bg-[#050507] min-h-screen text-slate-900 dark:text-white font-sans selection:bg-rose-500/30 overflow-x-hidden pt-20 transition-colors duration-500">
      ...
      <section className="min-h-screen flex flex-col justify-center px-6 lg:px-12 py-20 relative overflow-hidden">
        <div className="absolute inset-0 z-0 pointer-events-none opacity-20 dark:opacity-40">
           <div className="absolute top-0 right-0 w-[1000px] h-[1000px] bg-rose-500/10 rounded-full blur-[200px]"></div>
...
export default AINoelGenerator;
