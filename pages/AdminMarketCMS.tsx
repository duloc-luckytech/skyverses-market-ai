...
const AdminMarketCMS = () => {
  ...
  return (
    <div className="pt-32 pb-40 min-h-screen bg-white dark:bg-[var(--atlas-bg-page)] text-slate-900 dark:text-white font-sans transition-colors duration-500 overflow-x-hidden selection:bg-brand-blue/30">
        ...
        <header className="flex flex-col md:flex-row justify-between items-end gap-8">
           <div className="space-y-4">
              <div className="flex items-center gap-3 text-brand-blue">
                 <Settings2 size={24} />
                 <span className="text-[12px] font-bold uppercase tracking-[0.6em] italic">Infrastructure_Admin</span>
              </div>
              <h1 className="text-5xl lg:text-7xl font-bold uppercase tracking-tighter italic leading-none">Market <span className="text-brand-blue">Registry.</span></h1>
              
              <div className="flex bg-slate-100 dark:bg-white/[0.06] p-1 rounded-xl border border-black/5 dark:border-white/10 w-fit mt-4">
                <button 
                  onClick={() => setActiveTab('REGISTRY')}
                  className={`px-6 py-2.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${activeTab === 'REGISTRY' ? 'bg-white dark:bg-[var(--atlas-bg-panel)] text-brand-blue shadow-lg' : 'text-gray-500'}`}
                >
                  Market Items
                </button>
                <button 
                  onClick={() => setActiveTab('EVENTS')}
                  className={`px-6 py-2.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${activeTab === 'EVENTS' ? 'bg-white dark:bg-[var(--atlas-bg-panel)] text-brand-blue shadow-lg' : 'text-gray-500'}`}
                >
                  Event Studio Config
                </button>
              </div>
           ...
           <div className="relative group w-full md:w-96">
              ...
              <input 
                ...
                className="w-full bg-white dark:bg-black/20 border border-black/5 dark:border-white/10 rounded-lg pl-14 pr-6 py-4 text-sm font-bold focus:border-brand-blue outline-none transition-all shadow-sm"
              />
           </div>
        </header>
            ...
            <motion.div key="registry" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-16">
                ...
                <section className="space-y-8">
                  <div className="flex items-center gap-4 px-1">
                      ...
                      <div className="space-y-0.5">
                        <h3 className="text-xl font-bold uppercase italic tracking-tighter text-slate-900 dark:text-white">Registry Mismatches</h3>
                      ...
                      {comparison.missing.map((sol) => (
                        <div key={sol.id} className="p-8 bg-amber-500/[0.03] border border-amber-500/20 rounded-xl space-y-8">
                          <div className="flex justify-between items-start">
                              ...
                              <button onClick={() => handlePushToCloud(sol)} className="px-6 py-2.5 bg-amber-500 text-black rounded-xl text-[10px] font-bold uppercase tracking-widest shadow-atlas-lg">PUSH</button>
                          </div>
                        </div>
                      ))}
                  </div>
              ...
              <section className="space-y-8 pb-32">
                <div className="flex items-center gap-4 px-1">
                  ...
                  <h3 className="text-xl font-bold uppercase italic tracking-tighter text-slate-900 dark:text-white">Live Inventory</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {comparison.live.map((sol) => (
                    <div key={sol.id} className={`p-8 bg-white dark:bg-[var(--atlas-bg-panel)] border rounded-xl space-y-6 ${sol.isActive ? 'border-black/5 dark:border-white/[0.06]' : 'opacity-40 grayscale'}`}>
                       <div className="flex justify-between items-start">
                          ...
                          <div className="flex gap-1.5">
                             <button onClick={() => handleToggleActive(sol)} className="p-2 bg-slate-100 dark:bg-white/[0.06] rounded-xl"><Zap size={14} fill={sol.isActive ? "currentColor" : "none"} /></button>
                          </div>
                       </div>
                       <div className="aspect-video rounded-lg overflow-hidden bg-slate-100 dark:bg-black/40"><img src={sol.imageUrl} className="w-full h-full object-cover" alt="" /></div>
                    </div>
                  ))}
                </div>
              </section>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  ...
                  {(Object.values(localEventConfigs) as EventConfig[]).map((event) => {
                    ...
                    return (
                      <div 
                        key={event.id}
                        className={`p-8 bg-white dark:bg-[var(--atlas-bg-panel)] border-2 rounded-xl space-y-6 shadow-sm transition-all group hover:shadow-atlas-lg ${editingEvent?.id === event.id ? `border-${event.accentColor}-500` : 'border-black/5 dark:border-white/[0.06]'}`}
                      >
                         <div className="flex justify-between items-start">
                            <div className={`w-14 h-14 rounded-lg flex items-center justify-center border shadow-inner bg-${event.accentColor}-500/10 border-${event.accentColor}-500/20 text-${event.accentColor}-500 group-hover:scale-110 transition-transform`}>
                               <EvIcon size={28} />
                            </div>
                            <button 
                              onClick={() => setEditingEvent(event)}
                              className="p-2 text-slate-400 hover:text-brand-blue transition-colors bg-slate-50 dark:bg-white/[0.06] rounded-full"
                            >
                               <Edit3 size={16} />
                            </button>
                         </div>
                         <div className="space-y-1">
                            <h4 className="text-2xl font-bold uppercase italic tracking-tighter text-slate-900 dark:text-white leading-none">{event.name}</h4>
                            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{event.version}</p>
                         </div>
                         <div className="flex items-center justify-between pt-4 border-t border-black/5 dark:border-white/[0.06]">
                            <div className="flex items-center gap-2">
                               <Coins size={14} className="text-orange-500" />
                               <span className="text-sm font-bold italic">{event.costBase} CR</span>
                            </div>
                            <span className={`text-[8px] font-bold uppercase px-2 py-1 rounded bg-${event.accentColor}-500/10 text-${event.accentColor}-500`}>{event.id}</span>
                         </div>
                      </div>
                    );
                  })}
                 ...
                 {editingEvent && (
                   ...
                   >
                     <motion.div 
                       ...
                       className="bg-white dark:bg-[var(--atlas-bg-panel)] border border-black/10 dark:border-white/10 rounded-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-atlas-xl"
                     >
                        <div className="p-8 border-b border-black/5 dark:border-white/[0.06] flex items-center justify-between shrink-0">
                           <div className="flex items-center gap-4">
                              ...
                              <h3 className="text-xl font-bold uppercase italic tracking-tight">Cấu hình: {editingEvent.name}</h3>
                           </div>
                        ...
                        <div className="flex-grow overflow-y-auto p-8 lg:p-10 no-scrollbar space-y-10">
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                              <div className="space-y-2">
                                 <label className="text-[10px] font-bold uppercase text-gray-500 tracking-widest px-1">Tên Hiển Thị</label>
                                 <input 
                              ...
                              <div className="space-y-2">
                                 <label className="text-[10px] font-bold uppercase text-gray-500 tracking-widest px-1">Chi Phí Cơ Bản</label>
                           ...
                           <div className="space-y-2">
                              <label className="text-[10px] font-bold uppercase text-gray-500 tracking-widest px-1">Chủ Đề (Cách nhau bởi dấu phẩy)</label>
                              <textarea 
                           ...
                           <div className="space-y-2">
                              <label className="text-[10px] font-bold uppercase text-gray-500 tracking-widest px-1">Bối Cảnh (Cách nhau bởi đóng phẩy)</label>
                              <textarea 
                           ...
                           <div className="space-y-2">
                              <label className="text-[10px] font-bold uppercase text-gray-500 tracking-widest px-1">Base Prompt Directive</label>
                              <textarea 
                        ...
                        <div className="p-8 border-t border-black/5 dark:border-white/[0.06] flex justify-end gap-4 bg-slate-50 dark:bg-black/40 shrink-0">
                           <button onClick={() => setEditingEvent(null)} className="px-8 py-4 text-[10px] font-bold uppercase tracking-widest text-gray-500 hover:text-slate-900 dark:hover:text-white transition-colors">Hủy Bỏ</button>
                           <button 
                             onClick={handleSaveEventConfig}
                             className={`px-12 py-4 bg-${editingEvent.accentColor}-600 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest shadow-atlas-lg hover:brightness-110 active:scale-95 transition-all flex items-center gap-2`}
                           >
