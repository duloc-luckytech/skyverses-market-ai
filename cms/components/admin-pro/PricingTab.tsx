      ...
      <div className="shrink-0 bg-white dark:bg-[#08080a] border-b border-slate-200 dark:border-white/5 p-6 lg:p-8 space-y-5">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="ml-auto flex items-center gap-3">
            <button onClick={fetchData} className="flex items-center gap-2 px-4 py-2 text-[10px] font-bold uppercase text-slate-400 hover:text-slate-900 dark:hover:text-white bg-slate-50 dark:bg-white/5 rounded-lg border border-slate-100 dark:border-white/5 hover:border-slate-300 dark:hover:border-white/20 transition-all">
              <RefreshCw size={12} className={loading ? 'animate-spin' : ''} /> Làm mới
            </button>
            <button onClick={handleAddNew} className="flex items-center gap-2 px-5 py-2.5 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-[10px] font-bold uppercase tracking-wider shadow-lg shadow-violet-600/20 hover:scale-[1.02] transition-all">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="relative flex-grow max-w-md">
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg pl-9 pr-4 py-2.5 text-xs outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500/20 text-slate-800 dark:text-white transition-all"
            />
          </div>
          ...
          <div className="flex bg-slate-100 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10 p-0.5">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wider transition-all ${toolFilter === t.key ? 'bg-white dark:bg-white/10 text-violet-600 dark:text-violet-400 shadow-sm' : 'text-slate-500 hover:text-slate-800 dark:hover:text-white'}`}
              >
                {t.icon}{t.label}
              </button>
            ))}
          ...
          <div className="flex bg-slate-100 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10 p-0.5">
            {['', 'gommo', 'fxlab', 'wan'].map(e => (
              <button
                ...
                className={`px-3.5 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wider transition-all ${engineFilter === e ? 'bg-white dark:bg-white/10 text-violet-600 dark:text-violet-400 shadow-sm' : 'text-slate-500 hover:text-slate-800 dark:hover:text-white'}`}
              >
                {e === '' ? 'Tất cả' : e}
              </button>
            ))}
          ...
          <div className="flex bg-slate-100 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10 p-0.5">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wider transition-all ${statusFilter === s.key ? 'bg-white dark:bg-white/10 text-violet-600 dark:text-violet-400 shadow-sm' : 'text-slate-500 hover:text-slate-800 dark:hover:text-white'}`}
              >
                {s.icon}{s.label}
              </button>
            ))}
      ...
      <AnimatePresence>
        {isDrawerOpen && (
          <div className="fixed inset-0 z-[1100] flex justify-end">
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="relative w-full max-w-xl bg-white dark:bg-[#0c0c0e] h-full shadow-3xl flex flex-col border-l border-slate-200 dark:border-white/10 text-slate-900 dark:text-white">
              {/* Drawer Header */}
              <div className="p-6 border-b border-slate-200 dark:border-white/10 flex items-center justify-between shrink-0 bg-slate-50 dark:bg-black/40">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 bg-violet-500/10 flex items-center justify-center text-violet-500 rounded-lg"><DollarSign size={18} /></div>
                  <div>
                ...
                <Section title="Thông tin cơ bản">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <select value={payload.tool} onChange={e => setPayload({ ...payload, tool: e.target.value })} className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-3 rounded-lg text-xs font-semibold text-slate-900 dark:text-white">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex gap-2">
                      <input value={newModeInput} onChange={e => setNewModeInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && addMode()} placeholder="vd: fast, relaxed..." className="flex-grow bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-3 rounded-lg text-xs outline-none focus:border-violet-500 text-slate-900 dark:text-white" />
                      <button onClick={addMode} className="p-3 bg-violet-600 text-white rounded-lg hover:bg-violet-700 transition-colors"><Plus size={14} /></button>
                    </div>
                    {payload.modes && payload.modes.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        {payload.modes.map(m => (
                          <span key={m} className="px-2.5 py-1 bg-violet-500/10 text-violet-500 text-[10px] font-bold rounded-md flex items-center gap-1.5 border border-violet-500/20">
                            {m}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea value={payload.description} onChange={e => setPayload({ ...payload, description: e.target.value })} className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-3 rounded-lg text-xs text-slate-900 dark:text-white" rows={2} placeholder="Mô tả model..." />
                  </div>
                </Section>
              ...
              {/* Drawer Footer */}
              <div className="p-6 border-t border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-black/40 flex justify-between items-center gap-4 shrink-0">
                ...
                <button onClick={handleSave} disabled={isSaving} className="bg-violet-600 hover:bg-violet-700 text-white px-8 py-3 rounded-lg text-xs font-bold flex items-center gap-3 shadow-lg shadow-violet-600/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50">
...
const EngineGroup: React.FC<{
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-white/[0.06] overflow-hidden bg-white dark:bg-[#0a0a0c]">
      {/* Engine Header */}
      <div className="flex items-center justify-between px-6 py-4 bg-slate-50 dark:bg-white/[0.02] border-b border-slate-200 dark:border-white/[0.06]">
        <div className="flex items-center gap-3">
      ...
      <div className="flex items-center gap-4 px-6 py-3.5 hover:bg-slate-50/50 dark:hover:bg-white/[0.015] transition-colors cursor-pointer" onClick={toggleExpand}>
        ...
        <div className="flex-grow min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className={`shrink-0 px-1.5 py-0.5 text-[8px] font-bold uppercase rounded border ${model.status === 'active' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-slate-200/50 dark:bg-white/5 text-slate-400 border-slate-200 dark:border-white/10'}`}>
              {model.status}
            </span>
          </div>
        ...
        <div className="hidden lg:flex items-center gap-2 shrink-0">
          ...
          {model.modes && model.modes.length > 0 && (
            <div className="flex items-center gap-1">
              {model.modes.slice(0, 3).map(m => (
                <span key={m} className="px-2 py-0.5 text-[9px] font-semibold rounded bg-violet-500/10 text-violet-500 border border-violet-500/10">{m}</span>
              ))}
          ...
          <div className="flex items-center gap-1">
            {resolutions.slice(0, 3).map(r => (
              <span key={r} className="px-2 py-0.5 text-[9px] font-semibold rounded bg-slate-100 dark:bg-white/5 text-slate-500 border border-slate-200 dark:border-white/10">{r}</span>
            ))}
          </div>
        ...
        <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button onClick={e => { e.stopPropagation(); onEdit(); }} title="Chỉnh sửa" className="p-2 rounded-md hover:bg-violet-500/10 text-slate-400 hover:text-violet-500 transition-all"><Edit3 size={13} /></button>
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
            <div className="px-6 pb-5 pt-2">
              ...
              <div className="flex flex-wrap gap-2 mb-4 lg:hidden">
                {model.modes?.map(m => <span key={m} className="px-2 py-0.5 text-[9px] font-semibold rounded bg-violet-500/10 text-violet-500">{m}</span>)}
                ...
                {Object.entries(model.pricing || {}).map(([res, durMap]) => (
                  <div key={res} className="p-3 bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/[0.06] rounded-lg">
                    <div className="flex items-center gap-2 mb-2.5 pb-2 border-b border-slate-100 dark:border-white/[0.06]">
                      <Monitor size={11} className="text-violet-500" />
                      <span className="text-[10px] font-bold uppercase text-violet-500 tracking-wider">{res}</span>
                    </div>
                    <div className="space-y-1.5">
...
const EditableCell = ({ modelId, res, dur, initialValue, onUpdate }: any) => {
  ...
  return (
    <div className={`flex items-center gap-1 text-orange-500 font-bold relative ${isSaving ? 'animate-pulse' : ''}`}>
      ...
      <input type="number" value={value} onChange={e => setValue(e.target.value)} onBlur={handleBlur} className="w-14 bg-transparent border-none outline-none text-right text-xs focus:bg-violet-500/10 rounded px-1 transition-colors" />
...
const DrawerInput = ({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (v: string) => void; type?: string }) => (
  <div className="space-y-1.5">
    ...
    <input type={type} value={value || ''} onChange={e => onChange(e.target.value)} className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-3 rounded-lg text-xs font-semibold outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500/20 text-slate-900 dark:text-white transition-all" />
  </div>
);
