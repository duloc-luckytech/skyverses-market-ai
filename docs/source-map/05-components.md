# Components — `components/` (~340 files)

## Atlas design-system primitives — `components/atlas/`

> Atlas Cloud-inspired design system (gold #C9A84C, 4px radius, Manrope font). Use these in new pages, gradually migrate existing.

| File | Role |
|------|------|
| `AtlasButton.tsx` | Polymorphic CTA — variants: primary (gold gradient) / secondary / outline / ghost / danger; sizes sm/md/lg; renders `<Link>` if `to`, `<a>` if `href`, else `<button>` |
...
| `AtlasPill.tsx` | Badge — tones gold/orange/success/info/warning/neutral |
| `AtlasHero.tsx` | Hero pattern — eyebrow + title + subtitle + actions slots, decorative gold/warm blobs, stagger reveal |
...
| `homepage/` | `shared.tsx` (motion primitives: EASE, MotionChip, SectionHeader, Reveal, GoldButton/OutlineButton, DrawIcon, HoverCard, `AnimatedStat` count-up, `CostDrop`, `HomepageKeyframes`), `CreatorsShowcaseSection` (pillar ② — 3 real creator-tool cards: image gen → Character Sync → video gen, each clickable to its `/product/*` route + live-preview video card, "Demo" badges, no fabricated telemetry), `CmsBlockSection` (CMS HomeBlocks — pillar ②), `LatestUpdatesSection` (horizontal latest blog/update cards with dark-gold hover/scroll showcase), `SolutionSection` (pillar ① — AI solutions for business: integrate AI into existing systems or build new AI product A→Z; 4 HoverCards, CTA→/booking), `AppGameSection` (pillar ③ — mobile app & game dev pipeline + sprint/playable/AI-layer presentation; CTA→/booking), `AppGameBuildList` (filterable "What we can build" demo showcase; placeholder `BUILT_APPS`, category chips + platform pills + timeline), `TrustStripSection` (dark consolidated trust band — 4 animated factual stats), `FinalCtaSection`. _Option C order (HomepageV2Sections): Hero → Creators → CmsBlock → LatestUpdates → Solution → AppGame → TrustStrip → FinalCta._ (`PromptDemoSection` removed — duplicated Creators showcase.) |
