...
| `homepage/` | `shared.tsx` (motion primitives: EASE, MotionChip, SectionHeader, Reveal, GoldButton/OutlineButton, DrawIcon, HoverCard, `AnimatedStat` count-up, `CostDrop`, `HomepageKeyframes`), `CreatorsShowcaseSection` (image/video panels), `PromptDemoSection` (live prompt→result typewriter demo), `CmsBlockSection` (CMS HomeBlocks — pillar ②), `SolutionSection` (pillar ① — AI solutions for business: integrate AI into existing systems or build new AI product A→Z; 4 HoverCards, CTA→/booking), `AppGameSection` (pillar ③ — mobile app & game dev: value cards + go-to-store strip + filterable demo showcase, category chips + platform pills + timeline; PLACEHOLDER demo data in `BUILT_APPS`, swap for real list; CTA→/booking), `TrustStripSection` (dark consolidated trust band — 4 animated factual stats), `FinalCtaSection`. _Option C order (HomepageV2Sections): Hero → Creators → PromptDemo → CmsBlock → Solution → AppGame → TrustStrip → FinalCta._ |
