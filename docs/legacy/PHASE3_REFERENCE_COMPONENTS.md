# Phase 3 Reference Components — Complete Source Code

---

## REFERENCE 1: Complete AISuggestPanel.tsx (495 lines)

**File:** `components/workspace/AISuggestPanel.tsx`

This is the **GOLD STANDARD** collapsible, tabbed panel with streaming patterns.
...
### 1. **Open State Persistence with localStorage**
```typescript
const [isOpen, setIsOpen] = useState(() => {
  try { 
    return localStorage.getItem(PANEL_OPEN_KEY(productSlug)) !== 'false'; 
  }
  catch { 
...
useEffect(() => {
  try { 
    localStorage.setItem(PANEL_OPEN_KEY(productSlug), String(isOpen)); 
  }
  catch { /* ignore */ }
}, [isOpen, productSlug]);
```

**For Phase 3:** Replace `productSlug` with `'storyboard_ai_script_assistant'`

---

### 2. **Smooth Height Animation with Framer Motion**
```typescript
<AnimatePresence>
  {isOpen && (
    <motion.div
...
**Key:** `ease: [0.22, 1, 0.36, 1]` is a cubic-bezier for natural acceleration

---

### 3. **Tab-Based Content with Mode Switching**
```typescript
<AnimatePresence mode="wait">
  {activeTab === 'prompts' && <TabPane>{...}</TabPane>}
  {activeTab === 'styles' && <TabPane>{...}</TabPane>}
  {activeTab === 'templates' && <TabPane>{...}</TabPane>}
  {activeTab === 'smartfill' && <TabPane>{...}</TabPane>}
</AnimatePresence>
```

**`mode="wait"`:** Ensures previous tab exits before next enters
...
**Effect:** Creates waterfall entrance animation
...
## Summary: What to Copy for Phase 3

| Pattern | Location | Purpose |
|---------|----------|---------|
| **Collapsible Toggle** | Lines 138-154 | Header button with chevron |
| **Open State Persistence** | Lines 108-113, 126-129 | localStorage + useEffect |
| **Height Animation** | Lines 169-178 | Smooth expand/collapse |
| **Tab Bar** | Lines 180-201 | Multi-tab interface |
| **Tab Content Switch** | Lines 203-269 | AnimatePresence with mode="wait" |
| **Stagger Animation** | Lines 288-306 | List item entrance |
| **Loading State** | Lines 282-286 | Spinner + disabled button |
| **Dark Mode Support** | Throughout | `dark:` Tailwind classes |
