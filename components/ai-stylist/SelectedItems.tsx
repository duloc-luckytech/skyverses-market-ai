  ...
  return (
    <AnimatePresence>
      {hasIndividualSelection && (
        ...
        >
          <div className="bg-slate-100 dark:bg-[var(--atlas-bg-panel)] border border-black/5 dark:border-white/[0.06] rounded-xl p-6 space-y-6 shadow-sm dark:shadow-atlas-lg">
            <div className="flex justify-between items-center">
              <span className="text-[13px] font-bold text-slate-500 dark:text-gray-400">Selected items</span>
              <button className="text-[10px] font-bold uppercase bg-white dark:bg-white/[0.06] border border-black/10 dark:border-white/10 px-4 py-2 rounded-xl hover:bg-slate-900 dark:hover:bg-white hover:text-white dark:hover:text-black transition-all">
                Save as Outfit
              </button>
            </div>
            <div className="flex flex-wrap gap-3">
              {individualKeys.map(key => {
                ...
                return (
                  <div key={key} className="w-20 h-20 rounded-lg overflow-hidden border border-black/5 dark:border-white/10 bg-white dark:bg-[var(--atlas-bg-panel)] relative group">
