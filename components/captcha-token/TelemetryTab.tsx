    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
      <div className="bg-white dark:bg-[var(--atlas-bg-panel)] border border-black/5 dark:border-white/10 rounded-xl overflow-hidden shadow-atlas-lg transition-all">
         <div className="p-8 border-b border-black/5 dark:border-white/[0.06] flex justify-between items-center bg-slate-50/50 dark:bg-black/20">
            ...
            <button onClick={() => setLogs([])} className="text-[10px] font-bold text-gray-400 hover:text-red-500 transition-colors uppercase tracking-widest">Xoá nhật ký tạm</button>
         </div>
                  ...
                  {logs.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="py-32 text-center opacity-10">
                         ...
                         <p className="text-[10px] font-bold uppercase tracking-[0.5em]">No Active Logs</p>
                      </td>
                    </tr>
                  ) : logs.map(l => (
                    <tr key={l.id} className="group hover:bg-black/[0.01] dark:hover:bg-white/[0.01] text-[11px] font-bold">
                       ...
                       <td className="px-10 py-6 text-center">
                          <span className={`px-2 py-0.5 rounded text-[8px] font-bold shadow-sm ${l.action === 'VIDEO' ? 'bg-purple-500/10 text-purple-500' : 'bg-brand-blue/10 text-brand-blue'}`}>
                            {l.action}
                          </span>
                       </td>
                       <td className="px-10 py-6 text-center text-indigo-500 italic font-bold">{l.latency}</td>
                       ...
                       <td className="px-10 py-6 text-right text-orange-500 font-bold italic">-{l.cost} CR</td>
                    </tr>
                  ))}
               </tbody>
            </table>
