      ...
      <div className="w-full lg:w-[350px] shrink-0 flex flex-col border-r border-black/10 dark:border-white/5 bg-gray-50 dark:bg-[var(--atlas-bg-page)] overflow-y-auto no-scrollbar p-6 space-y-8">
        <div className="space-y-6">
          ...
          <div className="p-4 bg-white dark:bg-white/[0.02] border border-black/5 dark:border-white/5 space-y-6 shadow-sm">
            ...
            <div className="space-y-2">
              <label className="text-[7px] font-bold uppercase text-gray-400 dark:text-gray-700">Master_Resolution</label>
              <div className="grid grid-cols-2 gap-2">
                 {['4K_Standard', '8K_Ultra'].map(r => (
                   <button
                     key={r}
                     onClick={() => setResolution(r)}
                     className={`py-2 text-[8px] font-bold uppercase border transition-all ${resolution === r ? 'bg-brand-blue border-brand-blue text-white' : 'border-black/10 dark:border-white/10 text-gray-400'}`}
                   >
                     {r.split('_')[0]}
                   </button>
                 ))}
        ...
        <div className="space-y-4">
           ...
           <div className="p-4 border border-black/5 dark:border-white/5 space-y-4">
              ...
              <div className="flex items-center gap-3 text-green-500">
                 <ShieldCheck size={12} />
                 <span className="text-[8px] font-bold uppercase tracking-widest">Neural_Uplink_Secure</span>
              </div>
           </div>
        </div>
            ...
            <div className="w-full h-full bg-black border border-black/10 dark:border-white/10 relative overflow-hidden flex items-center justify-center group/view shadow-2xl">
              ...
              {isGenerating && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 backdrop-blur-md z-20">
                   ...
                   <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-white animate-pulse">Synthesizing_Blueprint...</p>
                </div>
              )}
          ...
          <div className="space-y-3">
            {archives.length === 0 ? (
              <div className="py-20 text-center border border-dashed border-black/10 dark:border-white/10 opacity-20">
                 ...
                 <p className="text-[8px] font-bold uppercase tracking-widest">Vault_Empty</p>
              </div>
            ) : (
              archives.map((v, i) => (
