    ...
    <div className="flex flex-col lg:flex-row h-full w-full bg-white dark:bg-[var(--atlas-bg-page)] overflow-hidden text-black dark:text-white">
      <div className="w-full lg:w-[350px] shrink-0 flex flex-col border-r border-black/10 dark:border-white/[0.06] bg-gray-50 dark:bg-[var(--atlas-bg-page)] overflow-y-auto no-scrollbar p-6 space-y-8">
        <div className="space-y-6">
          ...
          <div className="p-4 bg-white dark:bg-white/[0.02] border border-black/5 dark:border-white/[0.06] space-y-4 shadow-sm">
            <div className="space-y-1">
              <label className="text-[7px] font-bold uppercase text-gray-400 dark:text-gray-700">Designation</label>
              <input
                value={charName}
                onChange={(e) => setCharName(e.target.value.toUpperCase())}
                className="w-full bg-transparent border-b border-black/10 dark:border-white/10 text-brand-blue text-xs font-bold uppercase outline-none focus:border-brand-blue pb-1"
              />
            </div>
            <div className="space-y-1">
              <label className="text-[7px] font-bold uppercase text-gray-400 dark:text-gray-700">Morphology</label>
              <textarea
                value={charDescription}
                onChange={(e) => setCharDescription(e.target.value)}
                className="w-full h-16 bg-transparent border border-black/5 dark:border-white/[0.06] p-2 text-[10px] font-medium text-gray-600 dark:text-gray-400 focus:outline-none resize-none italic"
              />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <label className="text-[9px] font-bold uppercase text-gray-500 dark:text-gray-600 tracking-[0.4em]">Consistency_Matrix</label>
          <div className="space-y-1.5">
            {Object.entries(locks).map(([key, val]) => (
              <button
                key={key}
                onClick={() => toggleLock(key as any)}
                className={`w-full p-3 border flex justify-between items-center transition-all ${val ? 'border-brand-blue/40 bg-brand-blue/5 text-brand-blue' : 'border-black/5 dark:border-white/[0.06] text-gray-400 opacity-50'}`}
              >
                <span className="text-[8px] font-bold uppercase">{key}_Geometry</span>
        ...
        <div className="pt-6 border-t border-black/10 dark:border-white/[0.06]">
          <label className="text-[9px] font-bold uppercase text-gray-500 dark:text-gray-600 tracking-[0.4em] flex items-center gap-3 mb-4">
            <Fingerprint size={12} className="text-brand-blue" /> Anchors
          </label>
          <div className="grid grid-cols-2 gap-3">
            <div onClick={() => fileInputRef.current?.click()} className="aspect-square bg-black/[0.06] dark:bg-white/[0.06] border border-dashed border-black/10 dark:border-white/10 flex flex-col items-center justify-center cursor-pointer hover:border-brand-blue transition-all">
              <Upload size={16} className="text-gray-400 mb-1" />
              <span className="text-[7px] font-bold uppercase text-gray-400">Add_Ref</span>
            </div>
            <div className="aspect-square bg-black/[0.06] dark:bg-white/[0.06] border border-black/10 dark:border-white/10 overflow-hidden relative grayscale opacity-50">
      ...
      <div className="flex-grow flex flex-col bg-gray-100 dark:bg-[var(--atlas-bg-page)] relative overflow-hidden">
            ...
            <div className="w-full h-full bg-black border border-black/10 dark:border-white/10 relative overflow-hidden flex items-center justify-center group/view shadow-atlas-lg">
              ...
              {isGenerating && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 backdrop-blur-md z-20">
                   ...
                   <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-white animate-pulse">Orchestrating_Identity...</p>
                </div>
              )}
        ...
        <div className="h-32 border-t border-black/10 dark:border-white/[0.06] bg-white dark:bg-black p-4 flex gap-4 shrink-0 relative z-20 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] dark:shadow-none">
          <div className="flex-grow flex flex-col gap-2">
            <label className="text-[8px] font-bold uppercase text-gray-400 dark:text-gray-700 tracking-[0.3em] flex items-center gap-2"><Terminal size={12} className="text-brand-blue" /> Directive</label>
            <textarea
              value={scenePrompt}
          ...
          >
            ...
            <span className="text-[8px] font-bold uppercase tracking-[0.2em]">Synthesize</span>
          </button>
        </div>
      </div>
      ...
      <div className="hidden xl:flex w-[280px] shrink-0 flex flex-col bg-gray-50 dark:bg-[var(--atlas-bg-page)] border-l border-black/10 dark:border-white/[0.06] overflow-y-auto no-scrollbar p-6 space-y-8">
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              <span className="text-[7px] font-bold uppercase text-gray-400 dark:text-gray-800 tracking-widest">Outfit</span>
              <input
                value={outfit}
                onChange={(e) => setOutfit(e.target.value)}
                className="w-full bg-black/[0.06] dark:bg-white/[0.06] border border-black/10 dark:border-white/10 p-2 text-[9px] font-bold uppercase text-black dark:text-white outline-none"
              />
            </div>
            <div className="space-y-1">
              <span className="text-[7px] font-bold uppercase text-gray-400 dark:text-gray-800 tracking-widest">Style</span>
              <select
                value={style}
        ...
        <div className="pt-6 border-t border-black/10 dark:border-white/[0.06] space-y-6">
          <h3 className="text-[9px] font-bold uppercase tracking-[0.4em] text-gray-500 dark:text-gray-600 flex items-center gap-3"><HistoryIcon size={12} className="text-brand-blue" /> Archives</h3>
          <div className="space-y-3">
            {versions.slice(0, 3).map((v, i) => (
              <div key={i} onClick={() => setActiveImage(v.url)} className="flex items-center gap-3 group cursor-pointer">
                <div className="w-10 h-10 border border-black/10 dark:border-white/[0.06] overflow-hidden">
                  <img src={v.url} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all" />
                </div>
                <div className="flex-grow min-w-0">
                  <p className="text-[7px] font-bold text-black dark:text-white uppercase truncate">{v.metadata}</p>
