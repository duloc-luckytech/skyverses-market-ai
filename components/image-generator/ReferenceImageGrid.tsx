...
export const ReferenceImageGrid: React.FC<ReferenceImageGridProps> = ({
  ...
  const [isExpanded, setIsExpanded] = useState(references.length > 0);
    ...
    <div className="space-y-1.5">
      ...
      >
        ...
        {isExpanded
          ...
          : <ChevronDown size={12} className="text-slate-400 dark:text-slate-500 group-hover:text-brand-blue transition-colors" />
        }
      </button>
      ...
      {isExpanded && (
        <>
          ...
          >
            ...
            {isUploading && (
              <div className="relative aspect-square rounded-lg overflow-hidden border border-dashed border-brand-blue/30 bg-white/[0.02] animate-pulse flex items-center justify-center">
                {tempUrl && <img src={tempUrl} className="w-full h-full object-cover opacity-40 blur-[1px]" alt="Uploading" />}
                <div className="absolute inset-0 flex items-center justify-center"><Loader2 size={14} className="text-brand-blue animate-spin" /></div>
              </div>
            )}
            ...
            {currentCount < maxRefs && (
              <div className="relative aspect-square group">
                ...
                <div className="absolute inset-0 bg-[#1a1a1e] opacity-0 group-hover:opacity-100 transition-all flex flex-col p-0.5 gap-0.5 z-10 border border-white/[0.08] rounded-lg shadow-atlas-lg">
                  ...
                  <button onClick={onLibraryTrigger} className="flex-grow flex items-center justify-center gap-1 bg-white/[0.04] rounded text-[#666] hover:bg-brand-blue hover:text-white transition-all text-[8px]">
