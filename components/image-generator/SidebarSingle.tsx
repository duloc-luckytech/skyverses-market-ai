  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-1.5">
      <div className="flex justify-between items-center px-0.5">
        <p className="text-[11px] font-semibold uppercase text-white/70 tracking-wider flex items-center gap-1.5"><PenLine size={12} className="text-brand-blue" /> Prompt</p>
        <button
          onClick={() => setShowSuggestions(!showSuggestions)}
          className="flex items-center gap-1 text-[10px] font-medium text-brand-blue/80 hover:text-brand-blue transition-colors"
        >
          <Sparkles size={10} /> Gợi ý
          <ChevronDown size={10} className={`transition-transform ${showSuggestions ? 'rotate-180' : ''}`} />
        </button>
      </div>
      <p className="text-[10px] text-white/50 px-0.5 leading-relaxed">Mô tả chi tiết hình ảnh bạn muốn, bao gồm phong cách, ánh sáng, màu sắc, góc chụp.</p>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {QUICK_PROMPTS.map((q, i) => (
            <button
              ...
              className="p-2 rounded-lg border border-white/[0.08] bg-white/[0.025] hover:border-brand-blue/25 text-left transition-all"
            >
              <span className="text-sm">{q.emoji}</span>
              <p className="text-[10px] text-white/60 mt-1 line-clamp-2 leading-relaxed">{q.text}</p>
            </button>
          ))}
        </div>
      )}
