    ...
    <div className="h-full w-full flex flex-col lg:flex-row bg-[#fcfcfd] dark:bg-[#0c0c0e] text-slate-900 dark:text-white font-sans overflow-hidden transition-colors duration-500 relative">
      ...
      <GeneratorSidebar 
        ...
        handleBulkImport={() => {
          ...
          g.setBatchPrompts(lines.slice(0, 10));
          g.setBulkText('');
      ...
      <GeneratorViewport 
        ...
        onApplyExample={(ex) => {
          g.setPrompt(ex.prompt);
          g.setReferences([ex.source]);
          g.setActiveMode('SINGLE');
        }}
        onEdit={g.openEditor}
      ...
      <ImageLibraryModal 
        ...
        maxSelect={6 - g.references.length}
      />
        ...
        {g.showLowCreditAlert && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[1100] bg-black/80 backdrop-blur-md flex items-center justify-center p-6">
             <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="max-w-md w-full bg-white dark:bg-[#111114] p-12 border border-slate-200 dark:border-white/10 rounded-[2rem] text-center space-y-8 shadow-3xl transition-colors">
                <div className="w-24 h-24 bg-orange-500/10 border border-orange-500/20 rounded-full flex items-center justify-center mx-auto text-orange-500 shadow-3xl"><AlertTriangle size={48} /></div>
                <div className="space-y-4">
                   ...
                   <p className="text-sm text-slate-500 dark:text-gray-400 font-bold leading-relaxed uppercase tracking-tight">Chu trình này yêu cầu **{g.totalCost} credits**. <br />Số dư của bạn hiện không đủ.</p>
                </div>
                <div className="flex flex-col gap-4">
                   ...
                   <button onClick={() => g.setShowLowCreditAlert(false)} className="text-[10px] font-black uppercase text-slate-400 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white transition-colors tracking-widest italic">Bỏ qua</button>
                </div>
             </motion.div>
          </motion.div>
        )}
