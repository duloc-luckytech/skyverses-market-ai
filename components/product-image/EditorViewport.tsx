...
export const EditorViewport: React.FC<EditorViewportProps> = ({
  ...
  return (
    <div 
      className={`flex-grow flex flex-col relative bg-slate-100/50 dark:bg-[#0a0d14] overflow-hidden transition-colors ${activeTool === 'hand' ? 'cursor-grab active:cursor-grabbing' : (activeTool === 'pen' || activeTool === 'eraser') ? 'cursor-crosshair' : ''}`} 
    ...
    >
      ...
      {/* Zoom HUD */}
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-40 bg-white/90 dark:bg-[#13171f]/90 backdrop-blur-xl border border-slate-200 dark:border-white/[0.06] rounded-xl px-3 py-1.5 flex items-center gap-4 text-[11px] font-bold shadow-lg">
      ...
      <AnimatePresence>
        {isCropping && !isGenerating && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="absolute top-20 left-1/2 -translate-x-1/2 z-[100] flex flex-col items-center gap-2.5 w-full max-w-4xl pointer-events-none px-4">
            <div className="bg-white/95 dark:bg-[#13171f]/95 backdrop-blur-2xl border border-slate-200 dark:border-white/[0.06] rounded-xl p-1 flex items-center gap-0.5 pointer-events-auto shadow-xl overflow-x-auto no-scrollbar max-w-full">
              {ratioPresets.map(r => (
            ...
            <div className="bg-white/95 dark:bg-[#13171f]/95 backdrop-blur-2xl border border-slate-200 dark:border-white/[0.06] rounded-xl p-2.5 flex items-center gap-6 pointer-events-auto shadow-xl">
      ...
      <div className="flex-grow flex items-center justify-center p-4 pr-16 md:p-12 lg:pr-24 pb-32 lg:pb-12 overflow-auto no-scrollbar relative">
         <div className="relative shadow-2xl dark:shadow-[0_20px_60px_rgba(0,0,0,0.4)] transition-transform duration-300 flex items-center justify-center" style={{ transform: `translate(${panOffset.x}px, ${panOffset.y}px) scale(${zoom / 100})` }}>
            {result ? (
               <div ref={containerRef as any} className="relative inline-block bg-white dark:bg-[#0a0d14] border border-slate-200 dark:border-white/[0.06] rounded-lg transition-all overflow-hidden">
               ...
               <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center justify-center gap-6 p-10 md:p-20 rounded-2xl border-2 border-dashed border-slate-200 dark:border-white/[0.06] bg-white dark:bg-[#13171f]/50 cursor-pointer group hover:border-brand-blue/30 transition-all" onClick={onUploadClick}>
