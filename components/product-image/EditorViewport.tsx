...
export const EditorViewport: React.FC<EditorViewportProps> = ({
  ...
  return (
    <div 
      className={`flex-grow flex flex-col relative bg-slate-100/50 dark:bg-[var(--atlas-bg-page)] overflow-hidden transition-colors ${activeTool === 'hand' ? 'cursor-grab active:cursor-grabbing' : (activeTool === 'pen' || activeTool === 'eraser') ? 'cursor-crosshair' : ''}`} 
    ...
    >
      ...
      {/* Zoom HUD */}
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-40 bg-white/90 dark:bg-[var(--atlas-bg-panel)]/90 backdrop-blur-xl border border-slate-200 dark:border-white/[0.06] rounded-xl px-3 py-1.5 flex items-center gap-4 text-[11px] font-bold shadow-lg">
