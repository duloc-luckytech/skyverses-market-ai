...
export const EditorSidebar: React.FC<EditorSidebarProps> = (props) => {
  ...
  return (
    <aside 
      className={`fixed lg:relative bottom-0 lg:top-0 left-0 w-full lg:w-80 bg-white dark:bg-[var(--atlas-bg-panel)] border-t lg:border-t-0 lg:border-r border-slate-100 dark:border-white/[0.04] flex flex-col z-[150] lg:z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.06)] lg:shadow-none transition-all duration-500 ease-in-out ${props.isMobileExpanded ? 'h-[80dvh] rounded-t-[2rem]' : 'h-14 lg:h-full lg:rounded-none'}`}
    >
      ...
      {/* Tabs */}
      <div className={`flex border-b border-slate-100 dark:border-white/[0.04] bg-white dark:bg-[var(--atlas-bg-panel)] shrink-0 ${!props.isMobileExpanded ? 'hidden lg:flex' : 'flex'}`}>
