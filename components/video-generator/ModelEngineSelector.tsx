    ...
    <div className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            <select 
              ...
              className="w-full bg-white dark:bg-[#1a1f2b] border border-slate-200 dark:border-white/10 p-3 rounded-lg text-[11px] font-black uppercase outline-none appearance-none focus:border-purple-500 text-slate-800 dark:text-white transition-colors cursor-pointer"
            >
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        <div className="space-y-1.5 text-center">
          ...
          <button onClick={cycleRatio} className="w-full py-2 border rounded-sm text-[8px] font-black uppercase transition-all bg-white dark:bg-[#1a1f2b] border-slate-200 dark:border-white/5 text-slate-400 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white hover:border-brand-blue">{ratio}</button>
        </div>
        ...
        <div className="space-y-1.5 text-center">
          ...
          <button onClick={cycleSound} className={`w-full py-2 border rounded-sm text-[8px] font-black uppercase transition-all ${soundEnabled ? 'bg-purple-500 text-white border-purple-400 shadow-lg shadow-purple-500/20' : 'bg-white dark:bg-[#1a1f2b] border-slate-200 dark:border-white/5 text-slate-400 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white hover:border-brand-blue'}`}>{soundEnabled ? 'Bật' : 'Tắt'}</button>
        </div>

        {/* Độ phân giải */}
        <div className="space-y-1.5 text-center">
          <p className="text-[8px] font-black uppercase text-slate-400 dark:text-gray-600">P.Giải</p>
          <button onClick={cycleResolution} className="w-full py-2 border rounded-sm text-[8px] font-black uppercase transition-all bg-white dark:bg-[#1a1f2b] border-slate-200 dark:border-white/5 text-slate-400 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white hover:border-brand-blue">{resolution}</button>
        </div>
      </div>
    </div>
  );
