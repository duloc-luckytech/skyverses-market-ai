...
const HeroBG: React.FC = () => (
  <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
    ...
    <motion.div
      ...
      style={{ background: 'radial-gradient(circle, rgba(229,199,103,0.05), transparent 70%)' }}
...
const PLACEHOLDER_GRADIENTS = [
  'from-brand-blue via-amber-500 to-yellow-700',
  'from-yellow-300 via-brand-blue to-amber-700',
  'from-sky-500 via-blue-600 to-indigo-700',
  'from-emerald-400 via-teal-500 to-cyan-600',
  'from-amber-400 via-orange-500 to-red-500',
  'from-zinc-600 via-brand-blue to-amber-600',
];
...
const IdleMosaic: React.FC<{ images?: string[] }> = ({ images = [] }) => (
  <div className="absolute inset-0 overflow-hidden">
    {/* Blurred orbs */}
    <motion.div
      className="absolute -top-10 -left-10 w-48 h-48 rounded-full bg-brand-blue/15 blur-3xl"
      animate={{ scale: [1, 1.15, 1], x: [0, 15, 0] }}
      transition={{ duration: 7, repeat: Infinity, ease: 'easeInOut' }}
    />
    <motion.div
      className="absolute -bottom-10 -right-10 w-40 h-40 rounded-full bg-brand-blue/10 blur-3xl"
    ...
    <div className="rounded-xl overflow-hidden border border-black/[0.06] dark:border-white/[0.07] bg-white dark:bg-[#0f0f0f]">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-black/[0.04] dark:border-white/[0.05]">
        ...
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded-full bg-brand-blue/90 hover:bg-brand-blue transition-colors cursor-default" />
      ...
      <div className="p-4 space-y-3 border-t border-black/[0.04] dark:border-white/[0.05]">
            ...
            <motion.button
              ...
              className={`px-3 py-1.5 rounded-full text-[11px] font-semibold transition-all duration-200 ${
                selectedPrompt === i && !customPrompt
                  ? 'bg-brand-blue/10 border border-brand-blue/40 text-brand-blue dark:text-brand-blue shadow-sm'
                  : 'bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.06] text-slate-500 dark:text-white/40 hover:border-brand-blue/30 hover:text-brand-blue dark:hover:text-brand-blue'
              }`}
            >
              {p.label}
            </motion.button>
        ...
        <div className="flex gap-2 items-center">
          ...
          <motion.button
            ...
            className="shrink-0 w-10 h-10 rounded-xl bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.07] flex items-center justify-center text-slate-400 hover:text-brand-blue hover:border-brand-blue/30 transition-colors"
        ...
        <div className="flex gap-2">
          <motion.button
            ...
            className={`relative flex-1 py-3 rounded-xl text-[12px] font-bold flex items-center justify-center gap-2 overflow-hidden transition-all ${
              isRunning
                ? 'bg-brand-blue/10 text-brand-blue/50 cursor-not-allowed'
                : 'bg-gradient-to-r from-brand-blue to-brand-blueHover text-black shadow-lg shadow-brand-blue/25 hover:shadow-brand-blue/40 hover:brightness-105'
            }`}
          >
            {/* Shimmer on hover */}
            {!isRunning && (
          ...
          <motion.button
            ...
            className="px-5 py-3 rounded-xl border border-brand-blue/25 dark:border-brand-blue/20 text-brand-blue dark:text-brand-blue text-[12px] font-semibold hover:bg-brand-blue/10 hover:border-brand-blue/45 transition-all whitespace-nowrap"
          >
            Studio →
          </motion.button>
        </div>
        ...
        <AnimatePresence>
          {!isAuthenticated && (
            ...
            >
              <button onClick={login} className="text-brand-blue font-bold hover:underline">Đăng nhập</button>
        ...
        <div className="lg:col-span-6 space-y-7">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            ...
            <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold leading-[1.02] tracking-tight">
              Image <br />
              <span className="text-brand-blue dark:text-brand-blue relative">
                Studio
                <motion.span
            ...
            <div className="flex flex-wrap gap-1.5">
              {MODEL_FAMILIES.map(m => (
                <div key={m.name} className={`px-2.5 py-1 rounded-lg text-[9px] font-medium border transition-all ${
                  m.hot ? 'bg-brand-blue/10 border-brand-blue/25 text-brand-blue dark:text-brand-blue'
          ...
          <div className="grid grid-cols-2 gap-2">
            {SPECS.map(s => (
              <div key={s.label} className="p-3 bg-black/[0.02] dark:bg-white/[0.02] border border-black/[0.06] dark:border-white/[0.04] rounded-xl flex items-start gap-2.5 hover:border-brand-blue/20 transition-all">
                <div className="shrink-0 w-6 h-6 rounded-md bg-brand-blue/10 flex items-center justify-center text-brand-blue dark:text-brand-blue">{s.icon}</div>
                <div>
          ...
          <button
            ...
            className="bg-gradient-to-r from-brand-blue to-brand-blueHover text-black px-10 py-4 rounded-xl text-[11px] font-semibold uppercase tracking-widest shadow-lg shadow-brand-blue/20 hover:brightness-110 active:scale-[0.98] transition-all flex items-center gap-3 group"
          >
