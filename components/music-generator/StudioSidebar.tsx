    ...
    >
      ...
      <div className="hidden lg:flex h-16 lg:h-20 items-center px-6 border-b border-black/5 dark:border-white/5 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-sm font-bold uppercase tracking-widest text-slate-900 dark:text-white italic leading-none">Music Studio</h2>
        </div>
      </div>
      ...
      <div className={`flex-grow overflow-y-auto no-scrollbar p-6 space-y-8 ${!props.isMobileExpanded ? 'hidden lg:block' : 'block'}`}>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => props.onExpand('desc')} className="text-[10px] font-bold text-brand-blue uppercase flex items-center gap-1 hover:underline">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => props.onExpand('lyrics')} className="text-[10px] font-bold text-brand-blue uppercase flex items-center gap-1 hover:underline">
              <Maximize2 size={12} /> Mở rộng
            </button>
          </div>
          <textarea 
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                   ...
                   <select 
                     ...
                     className="bg-transparent border-none p-0 text-[9px] font-bold uppercase text-slate-500 dark:text-gray-400 outline-none cursor-pointer hover:text-slate-900 dark:hover:text-white"
                   >
                     <option value="gommo" className="bg-white dark:bg-[var(--atlas-bg-panel)] text-slate-900 dark:text-white">Gommo</option>
                     <option value="fxlab" className="bg-white dark:bg-[var(--atlas-bg-panel)] text-slate-900 dark:text-white">FxLab</option>
                   </select>
                </div>
              </div>
                ...
                >
                  {props.availableModels.length > 0 ? (
                    props.availableModels.map(m => (
                      <option key={m._id} value={m._id} className="bg-white dark:bg-[var(--atlas-bg-panel)] text-slate-900 dark:text-white">{m.name}</option>
                    ))
                  ) : (
