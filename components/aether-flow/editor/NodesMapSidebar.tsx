...
export const NodesMapSidebar: React.FC<NodesMapSidebarProps> = ({ nodes, onToggleVisibility, isOpen, onToggle }) => {
  ...
  const NavItem = ({ id, icon: Icon, label }: { id: typeof activeMenu, icon: any, label: string }) => (
    <button 
      ...
      className={`w-12 h-12 flex items-center justify-center transition-all relative group ${activeMenu === id ? 'text-indigo-600 bg-indigo-500/5' : 'text-gray-500 hover:text-indigo-600 hover:bg-black/[0.06] dark:hover:bg-white/[0.06]'}`}
      title={label}
    >
      <Icon size={20} />
  ...
  return (
    <motion.aside 
      ...
      className="relative border-r border-black/5 dark:border-white/[0.06] bg-white dark:bg-[var(--atlas-bg-panel)] flex flex-row shrink-0 z-50 transition-all duration-300 min-w-[48px] h-full"
    >
      <div className="w-12 border-r border-black/5 dark:border-white/[0.06] flex flex-col items-center py-4 bg-slate-50 dark:bg-black/40 shrink-0 h-full">
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="p-5 border-b border-black/5 dark:border-white/[0.06] bg-slate-50 dark:bg-black/20 shrink-0 space-y-4">
              ...
              {activeMenu !== 'CHAT' && (
                <div className="relative group">
                  ...
                  <input 
                    ...
                    className="w-full bg-white dark:bg-black/40 border border-black/5 dark:border-white/[0.06] rounded-lg py-2.5 pl-9 pr-4 text-[10px] font-bold outline-none focus:border-indigo-500 transition-all"
                  />
                </div>
              )}
            </div>
                 ...
                 <div className="p-2 space-y-1">
                    {folders.map(folder => (
                       <div key={folder.id} className="space-y-1">
                          <button onClick={() => toggleFolder(folder.id)} className="w-full flex items-center justify-between p-3 hover:bg-black/[0.06] dark:hover:bg-white/[0.06] rounded-xl transition-all group">
                             <div className="flex items-center gap-3">
                             ...
                             <span className="text-[9px] font-bold bg-black/[0.06] dark:bg-white/[0.06] px-2 py-0.5 rounded-full">{folder.count}</span>
                          </button>
                          <AnimatePresence>
                             {folder.isOpen && folder.children && (
                                <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="ml-6 space-y-0.5 overflow-hidden">
                                   {folder.children.map((child, idx) => (
                                      <button key={idx} className="w-full flex items-center gap-3 p-2.5 hover:bg-black/[0.06] dark:hover:bg-white/[0.06] rounded-lg transition-all text-left group">
               ...
               {activeMenu === 'MAP' && (
                 <div className="p-2 space-y-1">
                    {filteredNodes.length > 0 ? filteredNodes.map((node) => (
                        <div key={node.id} className={`group flex items-center justify-between p-3 rounded-xl transition-all hover:bg-black/[0.06] dark:hover:bg-white/[0.06] ${node.hidden ? 'opacity-40' : 'opacity-100'}`}>
                    ...
                    <div className="flex-grow p-4 space-y-4 overflow-y-auto no-scrollbar min-h-0">
                       {chatMessages.map((msg, i) => (
                         <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                            <div className={`w-6 h-6 rounded flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-white/[0.06] text-indigo-600'}`}>
                               {msg.role === 'user' ? <User size={12} /> : <Bot size={12} />}
                            </div>
                            <div className={`max-w-[85%] p-3 rounded-xl text-[11px] leading-relaxed ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-slate-100 dark:bg-white/[0.06] text-slate-700 dark:text-gray-300 rounded-tl-none border border-black/5 dark:border-white/[0.06]'}`}>
                               {msg.content}
                            </div>
                         </div>
                       ))}
                       {isChatLoading && <div className="flex gap-3"><div className="w-6 h-6 rounded bg-slate-100 dark:bg-white/[0.06] flex items-center justify-center text-indigo-600 animate-pulse"><Bot size={12} /></div><div className="bg-slate-100 dark:bg-white/[0.06] p-3 rounded-xl rounded-tl-none border border-black/5 dark:border-white/[0.06]"><Loader2 size={12} className="animate-spin text-indigo-600" /></div></div>}
                       <div ref={chatEndRef} />
                    </div>
                    <div className="p-4 bg-slate-100 dark:bg-black/40 border-t border-black/5 dark:border-white/[0.06] shrink-0">
                       <form onSubmit={handleSendChat} className="relative">
                          <input value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder="Hỏi gì đó về quy trình..." className="w-full bg-white dark:bg-white/[0.06] border border-black/5 dark:border-white/10 rounded-xl py-3 pl-4 pr-10 text-[11px] outline-none focus:border-indigo-600 transition-all text-slate-900 dark:text-white" />
