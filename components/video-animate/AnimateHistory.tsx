  ...
  return (
    <>
      ...
      <motion.div 
        ...
        className="fixed bottom-0 left-0 right-0 h-64 bg-white/95 dark:bg-[var(--atlas-bg-page)]/95 backdrop-blur-3xl border-t border-slate-200 dark:border-white/10 z-[100] p-8 shadow-[0_-20px_100px_rgba(0,0,0,0.2)] dark:shadow-[0_-20px_100px_rgba(0,0,0,0.6)] transition-all duration-500"
      >
         <div className="max-w-[1500px] mx-auto flex flex-col gap-8">
            <div className="flex items-center justify-between">
               ...
               >
                  <div className="flex items-center gap-4">
                     ...
                     <div className="space-y-0.5">
                        <h3 className="text-lg font-bold uppercase italic tracking-tighter text-slate-900 dark:text-white">Registry Vault</h3>
                  ...
                  {processingCount > 0 && (
                     <div className="flex items-center gap-3 px-5 py-2 bg-cyan-500/10 border border-cyan-500/30 rounded-full">
                        ...
                        <span className="text-[10px] font-bold text-cyan-600 dark:text-cyan-500 uppercase tracking-[0.2em]">{processingCount} ĐANG TỔNG HỢP...</span>
                     </div>
                  )}
               </div>
               ...
               <div className="flex gap-4">
                  <button 
                    onClick={() => setIsExpanded(!isExpanded)}
                    className={`px-8 py-3 border border-slate-200 dark:border-white/10 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-slate-100 dark:hover:bg-white/10 transition-all flex items-center gap-3 ${isExpanded ? 'text-cyan-500 border-cyan-500/30' : 'text-slate-500 dark:text-gray-400'}`}
                  >
                  ...
                  <div key={task.id} className="relative group shrink-0 w-44 aspect-[3/4.2] bg-slate-50 dark:bg-[var(--atlas-bg-panel)] border border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden shadow-md transition-all hover:border-cyan-500/50">
                     ...
                     {task.status === 'processing' && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-white/60 dark:bg-black/60 backdrop-blur-sm">
                           ...
                           <span className="text-[8px] font-bold text-slate-800 dark:text-white uppercase tracking-[0.4em] text-center px-4 animate-pulse">Neural Synthesis...</span>
                        </div>
                     )}
                     ...
                     {task.status === 'error' && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-red-900/40 backdrop-blur-md">
                           ...
                           <span className="text-[9px] font-bold text-red-100 uppercase text-center px-4">Node Failure</span>
                        </div>
                     )}
