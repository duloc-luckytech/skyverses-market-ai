...
export const AnimateViewport: React.FC<Props> = ({ mode }) => {
  return (
    <div className="w-full max-w-[480px] flex flex-col h-full gap-8 overflow-y-auto no-scrollbar pb-32">
      <div className="bg-white dark:bg-[var(--atlas-bg-panel)] border border-slate-200 dark:border-white/10 rounded-xl p-10 space-y-10 shadow-atlas-lg transition-all">
        <div className="flex items-center gap-6">
          <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center shadow-atlas-lg transition-colors duration-500 ${mode === 'MOTION' ? 'bg-cyan-500 shadow-cyan-500/20' : 'bg-purple-600 shadow-purple-500/20'}`}>
        ...
        <div className="space-y-6 pt-6 border-t border-slate-100 dark:border-white/[0.06] transition-colors">
          ...
          <div className="grid grid-cols-1 gap-6">
            {DEMOS.map(demo => (
              <div key={demo.id} className="aspect-video bg-slate-100 dark:bg-black border border-slate-100 dark:border-white/[0.06] rounded-lg relative overflow-hidden group cursor-pointer shadow-atlas-lg">
                <video 
                  src={demo.video} 
                  autoPlay 
                  loop 
                ...
                <div className="absolute top-4 left-4">
                  <span className={`px-3 py-1 rounded-md text-[9px] font-bold uppercase text-white shadow-atlas-lg ${demo.color}`}>
                    {demo.label}
                  </span>
                </div>
        ...
        <div className="space-y-4 pt-6 border-t border-slate-100 dark:border-white/[0.06] transition-colors">
          <button className="w-full flex items-center justify-between p-6 bg-slate-50 dark:bg-white/[0.03] border border-slate-100 dark:border-white/[0.06] rounded-lg hover:bg-slate-100 dark:hover:bg-white/[0.06] transition-all group shadow-sm">
            <div className="flex items-center gap-5">
              <div className="w-12 h-12 rounded-lg bg-cyan-500/10 flex items-center justify-center text-cyan-600 dark:text-cyan-400 group-hover:scale-110 transition-transform shadow-inner">
                <Activity size={24} />
              </div>
          ...
          <button className="w-full flex items-center justify-between p-6 bg-slate-50 dark:bg-white/[0.03] border border-slate-100 dark:border-white/[0.06] rounded-lg hover:bg-slate-100 dark:hover:bg-white/[0.06] transition-all group shadow-sm">
            <div className="flex items-center gap-5">
              <div className="w-12 h-12 rounded-lg bg-purple-500/10 flex items-center justify-center text-purple-600 dark:text-purple-400 group-hover:scale-110 transition-transform shadow-inner">
                <UserCircle size={24} />
              </div>
              <div className="text-left space-y-0.5">
      ...
      <div className="p-8 bg-brand-blue/5 border border-brand-blue/10 rounded-xl space-y-4">
