...
export const ParameterSettings: React.FC<ParameterSettingsProps> = ({
  ...
  const selectClass = "w-full bg-slate-50 dark:bg-white/[0.03] border border-slate-200/80 dark:border-white/[0.06] px-2.5 py-2 rounded-lg text-xs font-medium outline-none appearance-none focus:border-purple-500/40 transition-all cursor-pointer text-slate-700 dark:text-white/80 disabled:opacity-40 disabled:cursor-not-allowed";
  ...
  return (
    <div className="px-4 py-3 border-t border-slate-100 dark:border-white/[0.04]">
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="space-y-1">
          <p className="text-[9px] font-semibold uppercase text-slate-500 dark:text-slate-400 tracking-wider px-0.5 flex items-center gap-1">
            <Timer size={10} className="text-amber-400" /> Lượng
          </p>
          <div className="relative">
