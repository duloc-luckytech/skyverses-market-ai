...
export const ActionFooter: React.FC<ActionFooterProps> = ({ 
  ...
  return (
    <div className="p-6 border-t border-slate-100 dark:border-white/10 bg-white/80 dark:bg-[var(--atlas-bg-panel)]/80 backdrop-blur-md space-y-4 shrink-0 transition-colors">
      <div className="flex items-center justify-between px-1">
        <div className="flex flex-col">
          <span className="text-[8px] font-bold uppercase text-slate-400 dark:text-gray-600 tracking-widest mb-1 italic">Số dư Credits</span>
          <div className="flex items-center gap-1.5 text-brand-blue leading-none">
            ...
            <span className="text-[12px] font-bold italic">{credits.toLocaleString()} CR</span>
          </div>
        </div>
        <div className="text-right flex flex-col items-end">
          <span className="text-[8px] font-bold uppercase text-slate-400 dark:text-gray-600 tracking-widest mb-1 italic">Chi phí</span>
          <div className="flex items-center gap-1 text-orange-500 leading-none">
            <Zap size={12} fill="currentColor" />
            <span className="text-[11px] font-bold italic">{cost} CR</span>
          </div>
        </div>
      </div>
      ...
      {!hasEnoughCredits && hasSource && (
        <div className="flex items-center justify-center gap-2 text-amber-500 animate-pulse">
          ...
          <span className="text-[9px] font-bold uppercase tracking-widest">Không đủ Credits để thực thi</span>
        </div>
      )}
    </div>
  );
